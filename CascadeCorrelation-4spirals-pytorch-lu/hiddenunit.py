...
class hiddenUnits2(torch.nn.Module):
    ...
    def starttrain(self,maxepoches,xs,es):  #es datalen*outputsize
       ...
       # self.up = torch.Tensor([np.max(es,0)])  # avg up
       # self.low = torch.Tensor([np.min(es,0)])  # avg low
        self.up = torch.Tensor([np.max(es)])  # avg up
        self.low = torch.Tensor([np.min(es)])  # avg low
        ...
        self.optimizer = JDE3(maxepoches, xs, es, self.I)
        self.optimizer.startTrain()
        bestx2, loss2 = self.optimizer.getBest()
        self.optimizer = JDE4(maxepoches, xs, es, self.I)
        self.optimizer.startTrain()
        bestx3, loss3 = self.optimizer.getBest()
'''
        iter=1000
        #bestx=[bestx0,bestx1,bestx2,bestx3]
        #self.maxLoss=[loss0,loss1,loss2,loss3]
      ...
      #  print("loss:",loss)
        ...
        #[bestx1,bestx2],self.maxLoss=self.optimizer.getBest()
        #bestx,self.maxLoss=self.optimizer.getBest()
        ...
        with open("./data.txt", "w") as f:
            ...
            f.write(str(data3))
        '''
        tweights = bestx[:-1].reshape(-1, bestx[:-1].shape[0])
        bias=bestx[-1]
