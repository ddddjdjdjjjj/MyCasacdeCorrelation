...
class CascadeNet(torch.nn.Module):
    ...
    def get_features(self,x):
        ...
        with torch.no_grad():
          ...
          xs=torch.Tensor(xs.data)#清洗


        return xs.clone().detach()
    ...
    def check_io_convergence(self,train_loss,epoch,acc1):
        if epoch >= self.leastepoch and abs(train_loss[-1] - train_loss[-self.leastepoch] )< 0.0001:  # 训练次数>=2且损失值下降少于eps abs表波动
    ...
    def  add_hidden_unit(self,hiddenindex,xs,es):
        ...
        trainiter=hiddenunitspool.starttrain(hiddenindex,1000,xs,es)
