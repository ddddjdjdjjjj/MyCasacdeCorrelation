...
class MOEAD2():
    ...
    def __init__(self,maxepoches,dim,m,H,xs,es,hiddenus):
        ...
        self.LBOUND = np.full(( dim), -80)
        self.UBOUND = np.full(( dim), 80)
        self.Init_data()
   ...
   # def show(self):
    ...
    def getBest(self):
        ...
        best.append(self.Pop[0])
        loss.append(self.Pop_FV[0])
        best.append(self.Pop[-1])
        loss.append(self.Pop_FV[-1])
        for i in range(self.hiddenus-2):
    ...
    class Mean_vector():
        ...
        def get_mean_vectors(self):  # m2 H4
            H = self.H
            m = self.m
            sequence = []
            for ii in range(H):
