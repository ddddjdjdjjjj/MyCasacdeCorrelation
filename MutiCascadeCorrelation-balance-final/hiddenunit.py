...
class hiddenUnits2(torch.nn.Module):
    ...
    def starttrain(self,maxepoches,xs,es):  #es datalen*outputsize
        ...
        self.optimizer=MOEAD2(maxepoches,self.I,es.shape[1],2,xs,es,self.O)
        self.train_loss=[]
        self.optimizer.startTrain()
        iter=self.optimizer.getFinalGen()
        bestx,self.maxLoss=self.optimizer.getBest()

        #self.maxLoss=[loss1,loss2,loss3,loss4]
       # bestx=np.array([bestx1,bestx2,bestx3,bestx4])

        #self.optimizer2 = JDE(maxepoches, xs, es, self.I)  #比较

        #self.optimizer2.startTrain()

        #self.bestx2, self.maxLoss2 = self.optimizer2.getBest()
        #print("loss2:",self.maxLoss2)
        #self.maxLoss2= self.optimizer2.getBestd().tolist()
       ...
       # print("hidden unit maxloss:", self.maxLoss, "iter:", iter)
        ...
        #print("Single hidden unit maxloss:", self.maxLoss2)
        '''
...
class hiddenUnitsPool():
    ...
    def getBestUnit(self,hiddenindex):
        ...
        with open(self.file_path+"hidden_best-"+str(hiddenindex)+".txt", "w") as f:
            ...
            #data3=self.hiddenunits[bestindex].bestx2.tolist()
            ...
            f.write(str(data2) + "\n")
           # f.write(str(self.hiddenunits[bestindex].maxLoss2) + "\n")
