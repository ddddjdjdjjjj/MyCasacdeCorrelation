...
class hiddenUnit1(torch.nn.Module):
    ...
    def check_io_convergence(self,train_loss,epoch):
        if len(train_loss) >= 20 and abs(train_loss[-1] - train_loss[-10] )< 0.001:  # 训练次数>=2且损失值下降少于eps abs表波动
...
class hiddenUnitsPool():
    ...
    def starttrain(self,hiddenindex,maxepoches,xs,es):
        ...
        for i in range(self.K):
            trainiter=self.hiddenunits[i].starttrain(maxepoches, xs.data, es.data)
